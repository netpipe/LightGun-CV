...
class GreenBorderTracker
{
public:
    void setScreenCorners(const std::vector<cv::Point2f> &screen)
    {
    ...
    {
        t = std::max(4, std::min(t, 200));

        // Search radius is related to border thickness.
        // Too large => may pick wrong green edges.
        // Too small => cannot follow fast motion.
        searchRadius = std::max(10, std::min(48, t / 2 + 10));
    }
    double averageCornerDistance(const std::vector<cv::Point2f> &a,
                                 const std::vector<cv::Point2f> &b) const
    {
    ...
    void applyRawUpdate(const std::vector<cv::Point2f> &meas,
                        const cv::Size &sz,
                        float dt)
    {
        Q_UNUSED(sz);
        ...
        H = cv::findHomography(smoothed, screenCorners);
    ...
    bool isValid() const
    {
        return initialized && !H.empty() && framesLost <= maxLost;
    }
    ...
    bool update(const cv::Mat &bgr,
                const Params &p,
                const std::vector<cv::Point2f> &detectedQuad,
                bool detected)
    {
        if (screenCorners.size() != 4)
            return false;

        float dt = static_cast<float>(framesLost + 1);

        // If raw detection produced a valid quad, use it directly.
        if (detected && detectedQuad.size() == 4) {
            double area = std::fabs(cv::contourArea(detectedQuad));

            if (area > p.minArea) {
                applyRawUpdate(detectedQuad, bgr.size(), dt);
                return isValid();
            }
        }

        // Raw detection lost: coast for a short time.
        framesLost++;
        confidence *= 0.88;
        decayVelocity();

        if (framesLost <= maxLost) {
            std::vector<cv::Point2f> pred =
                predictCorners(static_cast<float>(framesLost));

            H = cv::findHomography(pred, screenCorners);
        } else {
            H = cv::Mat();
        }
...
private:
    ...
    void initFromCorners(const std::vector<cv::Point2f> &camQuad)
    {
        ...
        initialized = !H.empty();
        confidence = initialized ? 1.0 : 0.0;
        framesLost = 0;
    }

    std::vector<cv::Point2f> predictCorners(float dt) const
    {
        std::vector<cv::Point2f> pred(4);
        ...
        for (int i = 0; i < 4; ++i)
            pred[i] = smoothed[i] + velocity[i] * dt;

        return pred;
    }

    void applyUpdate(const std::vector<cv::Point2f> &cand,
                     double conf,
                     float dt)
    {
        std::vector<cv::Point2f> predicted = predictCorners(dt);
        std::vector<cv::Point2f> old = smoothed;

        if (smoothed.size() != 4 || velocity.size() != 4) {
            smoothed = cand;
            velocity.assign(4, cv::Point2f(0.0f, 0.0f));
        } else {
...
const float alpha = response;
            for (int i = 0; i < 4; ++i) {
                ...
                velocity[i] = (smoothed[i] - old[i]) / dt;
            }
        }
    ...
    double frameDiag(const cv::Size &sz) const
    {
        return std::sqrt(static_cast<double>(sz.width) * sz.width +
                         static_cast<double>(sz.height) * sz.height);
    }

    cv::Point2f centroid(const std::vector<cv::Point2f> &v) const
    {
        cv::Point2f c;
        ...
        if (!v.empty())
            c *= 1.0f / static_cast<float>(v.size());

        return c;
    }

    bool validCorners(const std::vector<cv::Point2f> &c,
                      const cv::Size &sz) const
    {
        if (c.size() != 4)
            return false;

        double diag = frameDiag(sz);
        double limit = 3.0 * diag;
        ...
        double frameArea = static_cast<double>(sz.width) * sz.height;

        if (area < 0.0002 * frameArea || area > 50.0 * frameArea)
            return false;

        return true;
    }

    bool cornersNear(const std::vector<cv::Point2f> &a,
    ...
    bool validateQuad(const std::vector<cv::Point2f> &quad,
                      const cv::Size &sz,
                      double minArea,
                      const std::vector<cv::Point2f> *expected) const
    {
        if (quad.size() != 4 || screenCorners.size() != 4)
            return false;

        double area = std::fabs(cv::contourArea(quad));
        double diag = frameDiag(sz);

        if (area < minArea)
            return false;

        if (area < 0.0002 * diag * diag)
            return false;

        float w = static_cast<float>(
            (cv::norm(quad[1] - quad[0]) +
             cv::norm(quad[2] - quad[3])) * 0.5);

        float h = static_cast<float>(
            (cv::norm(quad[3] - quad[0]) +
             cv::norm(quad[2] - quad[1])) * 0.5);

        if (w < 8.0f || h < 8.0f)
            return false;

        float sw = static_cast<float>(
            cv::norm(screenCorners[1] - screenCorners[0]));

        float sh = static_cast<float>(
            cv::norm(screenCorners[3] - screenCorners[0]));

        float screenAspect = sw / std::max(1.0f, sh);
        float aspect = w / h;

        // Allow perspective, but reject shapes that are too different.
        if (aspect < screenAspect * 0.35f ||
            aspect > screenAspect * 2.85f) {
            return false;
        }

        cv::RotatedRect rr = cv::minAreaRect(quad);
        double rrArea = static_cast<double>(rr.size.width) *
                        static_cast<double>(rr.size.height);

        if (rrArea <= 0.0 || area / rrArea < 0.65)
            return false;
        ...
        if (expected && expected->size() == 4) {
            ...
            for (int i = 0; i < 4; ++i)
                avgCornerDist += cv::norm(quad[i] - (*expected)[i]);
    ...
    void makeMask(const cv::Mat &bgr,
                  const Params &p,
                  cv::Mat &mask,
                  cv::Mat &edge) const
    {
        ...
        cv::Mat eroded;
        cv::erode(mask, eroded, kernelOpen);

        // Edge pixels of the green mask.
        cv::subtract(mask, eroded, edge);
    }

    void makeScreenSamples(std::vector<cv::Point2f> &samples,
                           std::vector<cv::Point2f> &inward) const
    {
        samples.clear();
        inward.clear();
        ...
        float inset = std::max(6.0f, std::min(w, h) * 0.05f);

        int n = std::max(2, samplesPerEdge);

        auto addEdge = [&](cv::Point2f a,
                           cv::Point2f b,
                           cv::Point2f inwardDir) {
            for (int i = 0; i < n; ++i) {
                float t = (n == 1) ? 0.0f : static_cast<float>(i) /
                                            static_cast<float>(n - 1);

                samples.push_back(a + (b - a) * t);
                inward.push_back(inwardDir);
            }
        };

        // Corners are expected as TL, TR, BR, BL.
        // Inward directions point into the border/screen area.
        addEdge(screenCorners[0], screenCorners[1], cv::Point2f(0.0f, inset));
        addEdge(screenCorners[1], screenCorners[2], cv::Point2f(-inset, 0.0f));
        addEdge(screenCorners[2], screenCorners[3], cv::Point2f(0.0f, -inset));
        addEdge(screenCorners[3], screenCorners[0], cv::Point2f(inset, 0.0f));
    }

    bool inside(const cv::Point2f &p,
                const cv::Size &sz,
                float margin = 0.0f) const
    {
        return p.x >= -margin &&
               p.y >= -margin &&
               p.x < sz.width + margin &&
               p.y < sz.height + margin;
    }

    bool findEdgePoint(const cv::Mat &edge,
                       const cv::Mat &mask,
                       const cv::Point2f &p0,
                       const cv::Point2f &inDir,
                       int radius,
                       const cv::Size &sz,
                       cv::Point2f &out,
                       int mode) const
    {
        auto tryOffset = [&](float offset) -> bool {
            cv::Point2f q = p0 + inDir * offset;
            ...
            cv::Point2f inner = q + inDir * 2.0f;
            cv::Point2f outer = q - inDir * 2.0f;

            bool innerGreen = false;
            bool outerGreen = false;

            int ix = cvRound(inner.x);
            int iy = cvRound(inner.y);

            if (ix >= 0 && iy >= 0 && ix < sz.width && iy < sz.height)
                innerGreen = mask.at<uchar>(iy, ix) != 0;

            int ox = cvRound(outer.x);
            int oy = cvRound(outer.y);

            if (ox >= 0 && oy >= 0 && ox < sz.width && oy < sz.height)
                outerGreen = mask.at<uchar>(oy, ox) != 0;

            // mode 0: strict outer border:
            //   green must be inward, non-green outward.
            //
            // mode 1: relaxed:
            //   only require green inward.
            if (mode == 0) {
        ...
        for (int d = 1; d <= radius; ++d) {
            if (tryOffset(static_cast<float>(d)))
                return true;
    ...
    int trackEdgeSamples(const cv::Mat &edge,
                         const cv::Mat &mask,
                         ...
                         int mode) const
    {
        ...
        if (predicted.size() != 4 || screenCorners.size() != 4)
            return 0;

        if (!validCorners(predicted, sz))
            return 0;

        cv::Mat Hs2c = cv::findHomography(screenCorners, predicted);

        if (Hs2c.empty())
            return 0;

        std::vector<cv::Point2f> samples;
        std::vector<cv::Point2f> inwardScreen;

        makeScreenSamples(samples, inwardScreen);

        if (samples.empty())
            return 0;

        std::vector<cv::Point2f> camSamples;
        cv::perspectiveTransform(samples, camSamples, Hs2c);
...
const int radius = dynamicSearchRadius(sz);
        ...
        std::vector<cv::Point2f> camInward;
        cv::perspectiveTransform(inwardScreenPts, camInward, Hs2c);

        int found = 0;

        for (size_t i = 0; i < camSamples.size(); ++i) {
            if (!std::isfinite(camSamples[i].x) ||
                !std::isfinite(camSamples[i].y)) {
                continue;
            }

            if (!inside(camSamples[i], sz, 20.0f))
                continue;

            cv::Point2f dir = camInward[i] - camSamples[i];
...
private:
    ...
    void calibrate()
    {
        ...
        tracker.setScreenCorners(border->screenCorners());
        tracker.setBorderThickness(spinThickness->value());
        tracker.reset();

        trackerWasValid = false;

        statusBar()->showMessage(
            "Calibrating tracker: keep the green border visible to the camera.",
            3000);
    }
...
//    }
    ...
    void grabFrame()
    {
        ...
        tracker.update(frame, p, quad, found);

        bool trackerOk = tracker.isValid();

        if (trackerOk && !trackerWasValid)
            statusBar()->showMessage("Tracker locked.", 2000);

        if (!trackerOk && trackerWasValid)
            statusBar()->showMessage("Tracker lost.", 2000);

        trackerWasValid = trackerOk;

        calibrated = trackerOk;
        homography = tracker.homography();

        cv::Mat display = frame.clone();

        // Draw raw detected contour candidate.
        if (found) {
            for (int i = 0; i < 4; ++i) {
                cv::line(display,
                         quad[i],
        ...
        // Draw tracked border.
        if (trackerOk) {
            std::vector<cv::Point2f> tc = tracker.corners();
            ...
            if (tc.size() == 4) {
                for (int i = 0; i < 4; ++i) {
                    ...
                    cv::circle(display, tc[i], 4, cv::Scalar(0, 0, 255), -1);
                }
            }

            std::string info =
                "track: " +
                std::to_string(static_cast<int>(tracker.confidenceValue() * 100.0)) +
                "%  lost:" +
                std::to_string(tracker.lostFrames());
        ...
        if (calibrated && !homography.empty()) {
            ...
            if (!out.empty() &&
                ...
                std::string aimText =
                    "Aim: " + std::to_string(lastAim.x()) +
                    "," + std::to_string(lastAim.y());
