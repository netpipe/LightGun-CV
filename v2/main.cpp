...
public:
                ...
                bool detected)
    {
      ...
      //  bool canReacquire = (!initialized || framesLost > 6 || confidence < 0.25);
        ...
        // Primary tracker: search along predicted border edges.
        std::vector<cv::Point2f> camPts;
        std::vector<cv::Point2f> screenPts;

        int pairs = trackEdgeSamples(edge,
                                     mask,
                                     predicted,
                                     camPts,
                                     screenPts,
        ...
        // If strict mode did not find enough points, try relaxed mode.
        if (pairs < 8) {
            camPts.clear();
            screenPts.clear();

            pairs = trackEdgeSamples(edge,
                                     mask,
                                     predicted,
                                     camPts,
                                     screenPts,
        ...
        if (pairs >= 6) {
            cv::Mat inliers;
            cv::Mat Hcand = cv::findHomography(camPts,
                                               screenPts,
                                               cv::RANSAC,
                                               3.0,
                                               inliers);
...
int minInliers = std::max(6, static_cast<int>(0.18 * pairs));
            if (!Hcand.empty() && inlierCount >= minInliers) {
                ...
                if (validCorners(cand, bgr.size()) &&
                    ...
                    double conf = static_cast<double>(inlierCount) /
                                  static_cast<double>(pairs);

                    if (relaxed)
                        conf *= 0.65;

                    applyUpdate(cand, conf, dt);
                    return isValid();
                }
            }
        }
